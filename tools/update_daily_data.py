...
def main() -> None:
    ...
    if current_rows:
        ...
        for row in current_rows:
            merged_play_odds = row.setdefault("play_odds", {})
            fixture_id = str(row.get("fixture_id") or "")
            for play_type, rows_by_fixture in play_odds_by_type.items():
                if rows_by_fixture.get(fixture_id):
                    merged_play_odds[play_type] = rows_by_fixture[fixture_id].get(play_type, rows_by_fixture[fixture_id])
            ...
            database_status = sync_database(
                {
                    ...
                    "analysis_archive": {"updatedAt": now_iso(), "matches": archive},
...
def parse_current_rows(html: str) -> list[dict[str, Any]]:
    ...
    for item in pattern.finditer(html):
        ...
        play_odds = parse_play_odds(body)
...
def parse_play_odds(body: str) -> dict[str, dict[str, float]]:
    ...
    for tag in re.findall(r"<[^>]+>", body or ""):
        ...
        play_type = aliases.get(attrs.get("data-type", "").lower())
        if not play_type:
            continue
        value = attrs.get("data-value")
        price = attrs.get("data-sp")
        if value is None or price is None or not re.fullmatch(r"\d+(?:\.\d+)?", price):
            continue
        plays.setdefault(play_type, {})[html_lib.unescape(value).strip()] = float(price)
